...
public static void AdicionarUsuarioEnpoints(this WebApplication app)
    {   
        ...
        grupo.MapPatch("/{id}/{senhaAnterior}/{senhaNova}", PatchAlterarSenhaAsync)
    ...
    private static async Task<IResult> PatchAlterarSenhaAsync(string id, string senhaAnterior, string senhaNova, AtletaContext db, IPasswordHasher<Usuario> hasher)
    {
        var obj = await db.Usuarios.FindAsync(Convert.ToInt64(id));
